...
CP2MapPublisher::CP2MapPublisher( QWidget *pParent ) : CP2MapPublisher( pParent, false ){}

CP2MapPublisher::CP2MapPublisher( QWidget *pParent, bool edit ) : QDialog( pParent )
{
    ...
    AO->retranslateUi(m_advancedOptionsWindow);
...
void CP2MapPublisher::OnOldApiSubmitItemDownload(RemoteStorageDownloadUGCResult_t  * pItem, bool pFailure){
    ...
    if(tempMap.isNull()) tempMap = QPixmap( ":/zoo_textures/InvalidImage.png" );
...
void CP2MapPublisher::OpenBSPFileExplorer()
{
    ...
    if(filePath.isEmpty()) return;
    if(!filePath.endsWith(".bsp")){ //sanity check, this shouldn't be possible.
